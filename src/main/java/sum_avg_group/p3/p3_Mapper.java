...
public class p3_Mapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (isContain) {
            String hireDate = values[4].trim();
            String name = values[1].trim();
            // key为部门名称, value为姓名+#+加入日期
            context.write(new Text(outputDept.getDept()), new Text(name + "#" + hireDate));
        }
    }
}